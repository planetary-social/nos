...
/// Mock Cashu proof structure for testing
/// In production, this would be replaced by the actual CashuSwift proof type
public struct CashuProof {
    let amount: Int
    let id: String
    let secret: String
    let C: String
...
public class CashuWallet {
    ...
    public func createWalletEvent(author: Author) throws -> Event {
        ...
        event.content = Data(walletPrivateKey.utf8).base64EncodedString()
        
        return event
    }
    
    /// Creates a NIP-60 token event containing encrypted Cashu proofs
    public func createTokenEvent(proofs: [CashuProof], mint: String, author: Author) throws -> Event {
        let event = Event(context: author.managedObjectContext!)
        ...
        // Encrypt the proofs using NIP-44
        // For now, we'll simulate encryption
        let proofsJSON = proofs.map { ["amount": $0.amount, "secret": $0.secret] }
        if let jsonData = try? JSONSerialization.data(withJSONObject: proofsJSON),
           let jsonString = String(data: jsonData, encoding: .utf8) {
            event.content = Data(jsonString.utf8).base64EncodedString()
        }
    ...
    public func createNutzap(amount: Int, recipientPubkey: String, mint: String, comment: String?, author: Author) throws -> Event {
        let event = Event(context: author.managedObjectContext!)
        ...
        // Create mock P2PK-locked proofs
        // In real implementation, these would be actual locked Cashu tokens
        let proofs = [
            ["amount": amount, "secret": "locked-secret", "C": "locked-C"]
        ]
        
        if let jsonData = try? JSONSerialization.data(withJSONObject: ["proofs": proofs]),
           let jsonString = String(data: jsonData, encoding: .utf8) {
