...
final class AuthorTests: CoreDataTestCase {
    ...
    @MainActor func test_mutualFolloweesCount_is_not_zero() throws {
        // Arrange
        ...
        // Alice and Bob follow Carl
        let aliceFollowsCarl = Follow(context: testContext)
        aliceFollowsCarl.source = alice
        aliceFollowsCarl.destination = carl

        let bobFollowsCarl = Follow(context: testContext)
        bobFollowsCarl.source = bob
        bobFollowsCarl.destination = carl

        carl.addToFollowers(aliceFollowsCarl)
        carl.addToFollowers(bobFollowsCarl)

        // Act
        let mutualFollowsCount = alice.mutualFolloweesCount(comparedTo: bob)

        // Assert
        XCTAssertEqual(mutualFollowsCount, 1)
    }

    @MainActor func test_mutualFolloweesCount_is_zero() throws {
        // Arrange
        let alice = try Author.findOrCreate(by: "alice", context: testContext)
        let bob = try Author.findOrCreate(by: "bob", context: testContext)
        let carl = try Author.findOrCreate(by: "carl", context: testContext)
        ...
        // Act
        let mutualFollowsCount = carl.mutualFolloweesCount(comparedTo: bob)

        // Assert
        XCTAssertEqual(mutualFollowsCount, 0)
    }

    @MainActor func test_sortByMutualFollowees() throws {
        // Arrange
        let alice = try Author.findOrCreate(by: "alice", context: testContext)
        let bob = try Author.findOrCreate(by: "bob", context: testContext)
        let carl = try Author.findOrCreate(by: "carl", context: testContext)
        ...
        // Alice and Bob follow Eve
        let aliceFollowsEve = Follow(context: testContext)
        aliceFollowsEve.source = alice
        aliceFollowsEve.destination = eve

        let bobFollowsEve = Follow(context: testContext)
        bobFollowsEve.source = bob
        bobFollowsEve.destination = eve

        eve.addToFollowers(aliceFollowsEve)
        eve.addToFollowers(bobFollowsEve)
